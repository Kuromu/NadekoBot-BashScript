...
choice=7
	...
	echo "5. Set up credentials.json (If you have downloaded Chrome already)"
	echo "6. Exit"
...
if [ $choice -eq 1 ] ; then
	...
	wget -N https://github.com/Kuromu/NadekoBot-BashScript/raw/chrome/nadeko_installer_latest.sh && bash "$root/nadeko_installer_latest.sh"
	echo ""
	bash "$root/linuxAIO.sh"
		...
		if [ $choice -eq 2 ] ; then
			...
			wget -N https://github.com/Kuromu/NadekoBot-BashScript/raw/chrome/nadeko_run.sh && bash "$root/nadeko_run.sh"
			echo ""
			echo "Welcome back to Chrome."
			sleep 2s
			bash "$root/linuxAIO.sh"
		else
			if [ $choice -eq 3 ] ; then
				...
				sleep 5s
				wget -N https://github.com/Kuromu/NadekoBot-BashScript/raw/chrome/NadekoAutoRestartAndUpdate.sh && bash "$root/NadekoAutoRestartAndUpdate.sh"
				echo ""
				...
				if [ $choice -eq 4 ] ; then
					...
					wget -N https://github.com/Kuromu/NadekoBot-BashScript/raw/chrome/nadekoautoinstaller.sh && bash "$root/nadekoautoinstaller.sh"
					echo ""
					echo "Welcome back..."
					sleep 2s
					bash "$root/linuxAIO.sh"
					...
					if [ $choice -eq 5 ] ; then
						echo ""
						echo
echo -e "Let's begin creating a new credentials.json file if you are about to run the Chrome for the first time. \n \nPlease read JSON Explanations in the guide... \n \nPress [Y] when you are ready to continue or [N] to exit."
while true; do
    read -p "[y/n]: " yn
    case $yn in
        [Yy]* ) clear; break;;
