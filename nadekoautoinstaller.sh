...
if [ "$BITS" = 32 ]; then
	...
	echo
	printf "\e[1;31mPlease check the TwinBot self-hosting guide for alternatives.\e[0m\n"
...
if [ "$supported" = 0 ]; then
	...
	printf "\e[1;31mContact TwinBot's support on Discord with screenshot.\e[0m\n"
	rm nadekoautoinstaller.sh
	exit 1
fi

if [ "$OS" = "Linux" ]; then
	echo -e "Your OS $OS $VER $ARCH probably can run Microsoft .NET Core. \nContact TwinBot's support on Discord with screenshot."
	rm nadekoautoinstaller.sh
	exit 1
fi

echo -e "Welcome to TwinBot Auto Prerequisites Installer. \nWould you like to continue? \nYour OS: $OS \nOS Version: $VER \nArchitecture: $ARCH"

while true; do
    read -p "[y/n]: " yn
    case $yn in
        [Yy]* ) clear; echo Running TwinBot Auto-Installer; sleep 2; break;;
...
if [ "$OS" = "Ubuntu" ]; then
echo "This installer will download all of the required packages for TwinBot. It will use about 350MB of space. This might take awhile to download if you do not have a good internet connection."
echo ""
...
echo
echo "TwinBot Prerequisites Installation completed..."
read -n 1 -s -p "Press any key to continue..."
sleep 2
